    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sashType, i) in SASH_TYPE" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sashType.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sashType)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sashType.sash_value }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashType.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashType.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sashType.created_by }} </td>
                        <td v-show="viewMode"> {{ sashType.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SASH_TYPE.filter(res => res.selected).map(res => res.id); 
            }
        },
