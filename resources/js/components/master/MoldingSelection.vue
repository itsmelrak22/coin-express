    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(moldingSelection, i) in MOLDING_SELECTION" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="moldingSelection.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(moldingSelection)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ moldingSelection.moldingSelectionCode }} </td>
                        <td v-show="viewMode"> {{ formatDate(moldingSelection.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(moldingSelection.updated_at) }} </td>
                        <td v-show="viewMode"> {{ moldingSelection.created_by }} </td>
                        <td v-show="viewMode"> {{ moldingSelection.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Molding Selection Name"
                                        name="moldingSelectionCode" 
                                        class="required"
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.moldingSelectionCode"
                                        outlined 
                                        dense 
                                        label="Molding Selection Name"
                                        name="moldingSelectionCode" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.MOLDING_SELECTION.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getMoldingSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/moldingSelection/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getMoldingSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/moldingSelection/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getMoldingSelection();
                        this.dialogUpdate = false;
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/moldingSelection/store',
                        data: formdata
                    }).then(() => {
                        this._getMoldingSelection();
