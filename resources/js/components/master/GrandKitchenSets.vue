    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in GRAND_KITCHEN_SETS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.grand_kitchen_set_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.grand_kitchen_set_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Grand Kitchen Set</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Grand Kitchen Set Name"
                                        name="grand_kitchen_set_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Grand Kitchen Set Value"
                                        name="grand_kitchen_set_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Grand Kitchen Set</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.grand_kitchen_set_name"
                                        outlined 
                                        dense 
                                        label="Grand Kitchen Set Name"
                                        name="grand_kitchen_set_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.grand_kitchen_set_value"
                                        outlined 
                                        dense 
                                        label="Grand Kitchen Set Value"
                                        name="grand_kitchen_set_value" 
        ...
        <app-footer :items="GRAND_KITCHEN_SETS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.GRAND_KITCHEN_SETS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/grand_kitchen_sets/store',
                        data: formdata
                    }).then(() => {
                        this._getGrandKitchenSets();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/grand_kitchen_sets/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getGrandKitchenSets();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/grand_kitchen_sets/delete',
