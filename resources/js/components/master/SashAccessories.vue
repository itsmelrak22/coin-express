    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sashAccessories, i) in SASH_ACCESSORIES" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sashAccessories.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sashAccessories)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sashAccessories.accessory_code }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashAccessories.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashAccessories.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sashAccessories.created_by }} </td>
                        <td v-show="viewMode"> {{ sashAccessories.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.accessory_code"
                                        outlined 
                                        dense 
                                        label="Sash Accessory Name"
                                        name="accessory_code" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SASH_ACCESSORIES.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/sashAccessories/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSashAccessories();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/sashAccessories/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSashAccessories();
                        this.dialogUpdate = false;
