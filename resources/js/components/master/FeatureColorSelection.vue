    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(featureColor, i) in FEATURE_COLOR" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="featureColor.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(featureColor)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ featureColor.feature_color_value }} </td>
                        <td> {{ featureColor.feature_color_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(featureColor.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(featureColor.updated_at) }} </td>
                        <td v-show="viewMode"> {{ featureColor.created_by }} </td>
                        <td v-show="viewMode"> {{ featureColor.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Feature Color Selection Code"
                                        name="feature_color_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Feature Color Selection Name"
                                        name="feature_color_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.feature_color_value"
                                        outlined 
                                        dense 
                                        label="Feature Color Selection Code"
                                        name="feature_color_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.feature_color_name"
                                        outlined 
                                        dense 
                                        label="Feature Color Selection Name"
                                        name="feature_color_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.FEATURE_COLOR.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getFeatureColorSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/featureColor/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getFeatureColorSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/featureColor/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getFeatureColorSelection();
                        this.dialogUpdate = false;
