    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in SMILE_KITCHEN_SETS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.smile_kitchen_set_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.smile_kitchen_set_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Smile Kitchen Sets</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smile Kitchen Set Name"
                                        name="smile_kitchen_set_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smile Kitchen Set Value"
                                        name="smile_kitchen_set_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit House Type Category</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smile_kitchen_set_name"
                                        outlined 
                                        dense 
                                        label="Smile Kitchen Set Name"
                                        name="smile_kitchen_set_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smile_kitchen_set_value"
                                        outlined 
                                        dense 
                                        label="Smile Kitchen Set Value"
                                        name="smile_kitchen_set_value" 
        ...
        <app-footer :items="SMILE_KITCHEN_SETS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SMILE_KITCHEN_SETS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/smile_kitchen_sets/store',
                        data: formdata
                    }).then(() => {
                        this._getSmileKitchenSets();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/smile_kitchen_sets/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSmileKitchenSets();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/smile_kitchen_sets/delete',
