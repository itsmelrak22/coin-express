    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sprayLena, i) in LENA_RACK" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sprayLena.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sprayLena)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sprayLena.lena_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(sprayLena.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sprayLena.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sprayLena.created_by }} </td>
                        <td v-show="viewMode"> {{ sprayLena.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create New Lena Rack</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.LENA_RACK.filter(res => res.selected).map(res => res.id); 
            }
        },
