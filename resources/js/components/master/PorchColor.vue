    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in PORCH_COLORS" :key="i">
                         <td v-show="editMode">
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.PORCH_COLORS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/porche_colors/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getPorchColors();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/porche_colors/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getPorchColors();
                        this.dialogUpdate = false;
