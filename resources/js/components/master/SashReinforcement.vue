    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sashReinforcement, i) in SASH_REINFORCEMENT" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sashReinforcement.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sashReinforcement)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sashReinforcement.reinforcement_code }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashReinforcement.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashReinforcement.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sashReinforcement.created_by }} </td>
                        <td v-show="viewMode"> {{ sashReinforcement.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Sash Reinforcement Name"
                                        name="reinforcement_code" 
                                        class="required"
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.reinforcement_code"
                                        outlined 
                                        dense 
                                        label="Sash Reinforcement Name"
                                        name="reinforcement_code" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SASH_REINFORCEMENT.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getSashReinforcement'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/sashReinforcement/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSashReinforcement();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/sashReinforcement/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSashReinforcement();
                        this.dialogUpdate = false;
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/sashReinforcement/store',
                        data: formdata
                    }).then(() => {
                        this._getSashReinforcement();
