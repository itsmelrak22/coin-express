    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in BALCONY_BELT_MAKEUP_TILES" :key="i">
                        <td v-show="editMode">
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Balcony Belt Tiles</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Balcony Belt Makeup Tiles"
                                        name="balcony_belt_name" 
                                        class="required"
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Balcony Belt Tiles</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.balcony_belt_name"
                                        outlined 
                                        dense 
                                        label="Balcony Belt Makeup Tiles"
                                        name="balcony_belt_name" 
...
import FloatingButtons from '../../modules/FloatingButtons.vue';
    export default {
        ...
        data: () => ({
            toolbar: {
                title: 'BALCONY BELT MAKEUP TILES',
                view: true,
                back: true,
                filter: false,
                master: false,
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.BALCONY_BELT_MAKEUP_TILES.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/balcony_belt_makeup_tiles/store',
                        data: formdata
                    }).then(() => {
                        this._getBalconyBeltMakeupTiles();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/balcony_belt_makeup_tiles/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getBalconyBeltMakeupTiles();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/balcony_belt_makeup_tiles/delete',
