    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(joineryColorSelection, i) in JOINERY_COLOR" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="joineryColorSelection.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(joineryColorSelection)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ joineryColorSelection.joinery_color_value }} </td>
                        <td> {{ joineryColorSelection.joinery_color_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(joineryColorSelection.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(joineryColorSelection.updated_at) }} </td>
                        <td v-show="viewMode"> {{ joineryColorSelection.created_by }} </td>
                        <td v-show="viewMode"> {{ joineryColorSelection.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Joinery Color Selection Code"
                                        name="joinery_color_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Joinery Color Selection Name"
                                        name="joinery_color_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.joinery_color_value"
                                        outlined 
                                        dense 
                                        label="Joinery Color Selection Code"
                                        name="joinery_color_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.joinery_color_name"
                                        outlined 
                                        dense 
                                        label="Joinery Color Selection Name"
                                        name="joinery_color_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.JOINERY_COLOR.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getJoineryColorSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/joineryColorSelection/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getJoineryColorSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/joineryColorSelection/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getJoineryColorSelection();
                        this.dialogUpdate = false;
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/joineryColorSelection/store',
                        data: formdata
                    }).then(() => {
                        this._getJoineryColorSelection();
