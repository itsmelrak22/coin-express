    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(fireproofFanovaEntrance, i) in FIREPROOF_FANOVA" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="fireproofFanovaEntrance.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(fireproofFanovaEntrance)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ fireproofFanovaEntrance.fireproof_value }} </td>
                        <td> {{ fireproofFanovaEntrance.fireproof_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(fireproofFanovaEntrance.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(fireproofFanovaEntrance.updated_at) }} </td>
                        <td v-show="viewMode"> {{ fireproofFanovaEntrance.created_by }} </td>
                        <td v-show="viewMode"> {{ fireproofFanovaEntrance.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Fire Proof Fanova Code"
                                        name="fireproof_value" 
                                        class="required"
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.fireproof_value"
                                        outlined 
                                        dense 
                                        label="Fire Proof Fanova Code"
                                        name="fireproof_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.fireproof_name"
                                        outlined 
                                        dense 
                                        label="Fire Proof Fanova Name"
                                        name="fireproof_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.FIREPROOF_FANOVA.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/fireproofFanovaEntrance/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getFireproofFanova();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/fireproofFanovaEntrance/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getFireproofFanova();
                        this.dialogUpdate = false;
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/fireproofFanovaEntrance/store',
                        data: formdata
                    }).then(() => {
                        this._getFireproofFanova();
