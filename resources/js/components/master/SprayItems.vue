    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sprayItems, i) in SPRAY_ITEMS" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sprayItems.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sprayItems)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sprayItems.spray_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(sprayItems.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sprayItems.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sprayItems.created_by }} </td>
                        <td v-show="viewMode"> {{ sprayItems.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SPRAY_ITEMS.filter(res => res.selected).map(res => res.id); 
            }
        },
