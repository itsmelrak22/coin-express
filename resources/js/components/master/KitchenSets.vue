    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in KITCHEN_SETS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.kitchen_set_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.kitchen_set_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Kitchen Set</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Kitchen Set Name"
                                        name="kitchen_set_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Kitchen Set Value"
                                        name="kitchen_set_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Kitchen Set</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.kitchen_set_name"
                                        outlined 
                                        dense 
                                        label="Kitchen Set Name"
                                        name="kitchen_set_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.kitchen_set_value"
                                        outlined 
                                        dense 
                                        label="Kitchen Set Value"
                                        name="kitchen_set_value" 
        ...
        <app-footer :items="KITCHEN_SETS.length"></app-footer>
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        selectedData(){
            ...
            return this.KITCHEN_SETS.filter(res => res.selected).map(res => res.id);
        }
    },
    ...
    methods: {
        ...
        formatDate(val){
            return new Date(val).toLocaleDateString('en-US', {timeZOne: 'Asia/Manila', hour12: false })
        },
        ...
        Store(){
            if(this.$refs.Store.validate()){
                ...
                axios({
                    method: 'POST',
                    url: '/api/kitchen_sets/store',
                    data: formdata
                }).then( () =>{
                    this._getKitchenSets();
        ...
        toggleUpdateDialog(item){
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/kitchen_sets/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getKitchenSets();
                        this.dialogUpdate = false;
