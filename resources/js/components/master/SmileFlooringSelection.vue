    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(smileFlooring, i) in SMILE_FLOORING" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="smileFlooring.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(smileFlooring)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ smileFlooring.smile_flooring_value }} </td>
                        <td> {{ smileFlooring.smile_flooring_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(smileFlooring.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(smileFlooring.updated_at) }} </td>
                        <td v-show="viewMode"> {{ smileFlooring.created_by }} </td>
                        <td v-show="viewMode"> {{ smileFlooring.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smile Flooring Selection Code"
                                        name="smile_flooring_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smile Flooring Selection Name"
                                        name="smile_flooring_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smile_flooring_value"
                                        outlined 
                                        dense 
                                        label="Smile Flooring Selection Code"
                                        name="smile_flooring_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smile_flooring_name"
                                        outlined 
                                        dense 
                                        label="Smile Flooring Selection Name"
                                        name="smile_flooring_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SMILE_FLOORING.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getSmileFlooringSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/smileFlooring/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSmileFlooringSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/smileFlooring/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSmileFlooringSelection();
                        this.dialogUpdate = false;
