    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(smartFlooring, i) in SMART_FLOORING" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="smartFlooring.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(smartFlooring)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ smartFlooring.smart_flooring_value }} </td>
                        <td> {{ smartFlooring.smart_flooring_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(smartFlooring.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(smartFlooring.updated_at) }} </td>
                        <td v-show="viewMode"> {{ smartFlooring.created_by }} </td>
                        <td v-show="viewMode"> {{ smartFlooring.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smart Flooring Selection Code"
                                        name="smart_flooring_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smart Flooring Selection Name"
                                        name="smart_flooring_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smart_flooring_value"
                                        outlined 
                                        dense 
                                        label="Smart Flooring Selection Code"
                                        name="smart_flooring_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smart_flooring_name"
                                        outlined 
                                        dense 
                                        label="Smart Flooring Selection Name"
                                        name="smart_flooring_name" 
...
import FloatingButtons from '../../modules/FloatingButtons.vue';
    export default {
        ...
        data: () => ({
            toolbar: {
                title: 'SMART FLOORING SELECTION',
                view: true,
                back: true,
                filter: false,
                master: false,
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SMART_FLOORING.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getSmartFlooringSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/smartFlooring/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSmartFlooringSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/smartFlooring/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSmartFlooringSelection();
                        this.dialogUpdate = false;
