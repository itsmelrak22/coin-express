    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(cubeFlooring, i) in CUBE_FLOORING" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="cubeFlooring.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(cubeFlooring)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ cubeFlooring.cube_flooring_value }} </td>
                        <td> {{ cubeFlooring.cube_flooring_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(cubeFlooring.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(cubeFlooring.updated_at) }} </td>
                        <td v-show="viewMode"> {{ cubeFlooring.created_by }} </td>
                        <td v-show="viewMode"> {{ cubeFlooring.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Cube Flooring Selection Code"
                                        name="cube_flooring_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Cube Flooring Selection Name"
                                        name="cube_flooring_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.cube_flooring_value"
                                        outlined 
                                        dense 
                                        label="Cube Flooring Selection Code"
                                        name="cube_flooring_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.cube_flooring_name"
                                        outlined 
                                        dense 
                                        label="Cube Flooring Selection Name"
                                        name="cube_flooring_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.CUBE_FLOORING.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getCubeFlooringSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/cubeFlooring/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getCubeFlooringSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/cubeFlooring/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getCubeFlooringSelection();
                        this.dialogUpdate = false;
