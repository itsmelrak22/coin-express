    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in EAVES_SELECTIONS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ITEM.eaves_name}} </td>
                        <td v-show="viewMode"> {{ITEM.eaves_value}} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Eaves</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Eaves</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.eaves_name"
                                        outlined 
                                        dense 
                                        label="Eaves Name"
                                        name="eaves" 
...
export default {
        ...
        computed: {
            ...
            selectedData(){
                ...
                return this.EAVES_SELECTIONS.filter(res => res.selected).map(res => res.id);
            }
        },
        ...
        methods: {
            ...
            formatDate(val){
                return new Date(val).toLocaleDateString('en-US', {timeZome: 'Asia/Manila', hour12: false})
            },
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/eaves_selections/store',
                        data: formdata
                    }).then( () =>{
                        this._getEavesSelections();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/eaves_selections/update/${id}`,
                        data: formdata
                    }).then( () =>{
                        this._getEavesSelections();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/eaves_selections/delete',
