    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in SLIT_SLIDERS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.slitslider_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.slitslider_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Slitslider</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Slitslider Name"
                                        name="slitslider_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Slitslider Value"
                                        name="slitslider_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Slitslider</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.slitslider_name"
                                        outlined 
                                        dense 
                                        label="Slitslider Name"
                                        name="slitslider_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.slitslider_value"
                                        outlined 
                                        dense 
                                        label="Slitslider Value"
                                        name="slitslider_value" 
        ...
        <app-footer :items="SLIT_SLIDERS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SLIT_SLIDERS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/slit_sliders/store',
                        data: formdata
                    }).then(() => {
                        this._getSlitsliders();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/slit_sliders/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSlitsliders();
                        this.dialogUpdate = false;
