    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(grandFlooring, i) in GRAND_FLOORING" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="grandFlooring.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(grandFlooring)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ grandFlooring.grand_flooring_value }} </td>
                        <td> {{ grandFlooring.grand_flooring_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(grandFlooring.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(grandFlooring.updated_at) }} </td>
                        <td v-show="viewMode"> {{ grandFlooring.created_by }} </td>
                        <td v-show="viewMode"> {{ grandFlooring.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Grand Flooring Selection Code"
                                        name="grand_flooring_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Grand Flooring Selection Name"
                                        name="grand_flooring_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.grand_flooring_value"
                                        outlined 
                                        dense 
                                        label="Grand Flooring Selection Name"
                                        name="grand_flooring_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.grand_flooring_name"
                                        outlined 
                                        dense 
                                        label="Grand Flooring Selection Name"
                                        name="grand_flooring_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.GRAND_FLOORING.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getGrandFlooringSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/grandFlooring/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getGrandFlooringSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/grandFlooring/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getGrandFlooringSelection();
                        this.dialogUpdate = false;
