<template>
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in GABLE_SELECTIONS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.gable_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.gable_value }} </td>
    ...
    <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Gable</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Gable</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.gable_name"
                                        outlined 
                                        dense 
                                        label="Gable Name"
                                        name="gable_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.GABLE_SELECTIONS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/gable_selections/store',
                        data: formdata
                    }).then(() => {
                        this._getGableSelections();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/gable_selections/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getGableSelections();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/gable_selections/delete',
