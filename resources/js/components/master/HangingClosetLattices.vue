    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in HANGING_CLOSET_LATTICES" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.hanging_closet_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.hanging_closet_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Hanging Closet Lattice</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Hanging Closet Lattice Name"
                                        name="hanging_closet_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Hanging Closet Lattice Value"
                                        name="hanging_closet_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Hanging Closet Lattice</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.hanging_closet_name"
                                        outlined 
                                        dense 
                                        label="Hanging Closet Lattice Name"
                                        name="hanging_closet_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.hanging_closet_value"
                                        outlined 
                                        dense 
                                        label="Hanging Closet Lattice Value"
                                        name="hanging_closet_value" 
        ...
        <app-footer :items="HANGING_CLOSET_LATTICES.length"></app-footer>
    </div>
</template>
<script>
...
import FloatingButtons from '../../modules/FloatingButtons.vue';
    export default {
        ...
        data: () => ({
            toolbar: {
                title: 'HP HANGING CLOSET LATTICES',
                view: true,
                back: true,
                filter: false,
                master: false,
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.HANGING_CLOSET_LATTICES.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/hanging_closet_lattices/store',
                        data: formdata
                    }).then(() => {
                        this._getHangingClosetLattices();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/hanging_closet_lattices/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getHangingClosetLattices();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/hanging_closet_lattices/delete',
