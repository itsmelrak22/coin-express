    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(fanovaEntranceDoor, i) in FANOVA_ENTRANCE" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="fanovaEntranceDoor.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(fanovaEntranceDoor)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ fanovaEntranceDoor.fanova_value }} </td>
                        <td> {{ fanovaEntranceDoor.fanova_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(fanovaEntranceDoor.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(fanovaEntranceDoor.updated_at) }} </td>
                        <td v-show="viewMode"> {{ fanovaEntranceDoor.created_by }} </td>
                        <td v-show="viewMode"> {{ fanovaEntranceDoor.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.FANOVA_ENTRANCE.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getFanovaEntranceDoor'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/fanovaEntrance/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getFanovaEntranceDoor();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/fanovaEntrance/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getFanovaEntranceDoor();
                        this.dialogUpdate = false;
