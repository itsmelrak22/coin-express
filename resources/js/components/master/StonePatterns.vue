    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in STONE_PATTERNS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.stone_pattern_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.stone_pattern_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Stone Pattern</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Stone Pattern Name"
                                        name="stone_pattern_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Stone Pattern Value"
                                        name="stone_pattern_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Stone Pattern</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.stone_pattern_name"
                                        outlined 
                                        dense 
                                        label="Stone Pattern Name"
                                        name="stone_pattern_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.stone_pattern_value"
                                        outlined 
                                        dense 
                                        label="Stone Pattern Value"
                                        name="stone_pattern_value" 
        ...
        <app-footer :items="STONE_PATTERNS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.STONE_PATTERNS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/stone_patterns/store',
                        data: formdata
                    }).then(() => {
                        this._getStonePatterns();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/stone_patterns/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getStonePatterns();
                        this.dialogUpdate = false;
