    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(houseType, i) in HOUSE_TYPES" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="houseType.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(houseType)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ houseType.name }} </td>
                        <td> {{ houseType.alias }} ( {{houseType.category}} ) </td>
                        <td v-show="viewMode"> {{ formatDate(houseType.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(houseType.updated_at) }} </td>
                        <td v-show="viewMode"> {{ houseType.created_by }} </td>
                        <td v-show="viewMode"> {{ houseType.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                ...
                                <v-col cols="12">
                                    <v-autocomplete 
                                        ...
                                        name="house_type_category_id" 
                                        class="required"
                                        :items="HOUSE_TYPE_CATEGORIES"
                                        item-text="category"
                                        item-value="id"
                                        :rules="rules.required"
                                    > </v-autocomplete>
                        ...
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                ...
                                <v-col cols="12">
                                    <v-autocomplete 
                                        v-model="tempData.house_type_category_id"
                                        outlined 
                                        dense 
                                        label="House Type Category"
                                        name="house_type_category_id" 
        ...
        <app-footer :items="HOUSE_TYPES.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.HOUSE_TYPES.filter(res => res.selected).map(res => res.id); 
            }
        },
