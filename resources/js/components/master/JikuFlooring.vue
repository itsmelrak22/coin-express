    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(jikuFlooring, i) in JIKU_FLOORING" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="jikuFlooring.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(jikuFlooring)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ jikuFlooring.jiku_flooring_value }} </td>
                        <td> {{ jikuFlooring.jiku_flooring_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(jikuFlooring.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(jikuFlooring.updated_at) }} </td>
                        <td v-show="viewMode"> {{ jikuFlooring.created_by }} </td>
                        <td v-show="viewMode"> {{ jikuFlooring.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Jiku Flooring Selection Code"
                                        name="jiku_flooring_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Jiku Flooring Selection Name"
                                        name="jiku_flooring_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.jiku_flooring_value"
                                        outlined 
                                        dense 
                                        label="Jiku Flooring Selection Code"
                                        name="jiku_flooring_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.jiku_flooring_name"
                                        outlined 
                                        dense 
                                        label="Jiku Flooring Selection Name"
                                        name="jiku_flooring_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.JIKU_FLOORING.filter(res => res.selected).map(res => res.id); 
            }
        },

        methods: {
            ...mapActions([
                '_getJikuFlooringSelection'
            ]),
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/jikuFlooring/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getJikuFlooringSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/jikuFlooring/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getJikuFlooringSelection();
                        this.dialogUpdate = false;
