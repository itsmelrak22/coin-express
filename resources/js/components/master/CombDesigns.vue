    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in COMB_DESIGNS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td>{{ITEM.comb_name}}</td>
                        <td v-show="viewMode">{{ITEM.comb_value}}</td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Comb</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Comb</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.comb_name"
                                        outlined 
                                        dense 
                                        label="Comb Name"
                                        name="comb_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.COMB_DESIGNS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/comb_designs/store',
                        data: formdata
                    }).then(() => {
                        this._getCombDesigns();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/comb_designs/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getCombDesigns();
                        this.dialogUpdate = false;
