    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in BASE_DRAINERS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.base_drainer_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.base_drainer_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Base Drainer</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Base Drainer Name"
                                        name="base_drainer_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Base Drainer Value"
                                        name="base_drainer_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Base Drainer</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.base_drainer_name"
                                        outlined 
                                        dense 
                                        label="Base Drainer Name"
                                        name="base_drainer_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.base_drainer_value"
                                        outlined 
                                        dense 
                                        label="Base Drainer Value"
                                        name="base_drainer_value"
        ...
        <app-footer :items="BASE_DRAINERS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.BASE_DRAINERS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/base_drainers/store',
                        data: formdata
                    }).then(() => {
                        this._getBaseDrainers();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/base_drainers/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getBaseDrainers();
                        this.dialogUpdate = false;
