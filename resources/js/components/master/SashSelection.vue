    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sashSelection, i) in SASH_SELECTION" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sashSelection.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sashSelection)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sashSelection.sashSelectionCode }} </td>
                        <td> {{ sashSelection.sashSelectionName }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashSelection.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sashSelection.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sashSelection.created_by }} </td>
                        <td v-show="viewMode"> {{ sashSelection.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Sash Selection Code"
                                        name="sashSelectionCode" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Sash Selection Name"
                                        name="sashSelectionName" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.sashSelectionCode"
                                        outlined 
                                        dense 
                                        label="Sash Selection Code"
                                        name="sashSelectionCode" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.sashSelectionName"
                                        outlined 
                                        dense 
                                        label="Sash Selection Name"
                                        name="sashSelectionName" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SASH_SELECTION.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/sashSelection/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSashSelection();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/sashSelection/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSashSelection();
                        this.dialogUpdate = false;
