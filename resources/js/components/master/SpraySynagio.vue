    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(spraySynagio, i) in SPRAY_SYNAGIO" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="spraySynagio.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(spraySynagio)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ spraySynagio.synagio_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(spraySynagio.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(spraySynagio.updated_at) }} </td>
                        <td v-show="viewMode"> {{ spraySynagio.created_by }} </td>
                        <td v-show="viewMode"> {{ spraySynagio.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SPRAY_SYNAGIO.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/spraySynagio/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSpraySynagio();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/spraySynagio/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSpraySynagio();
                        this.dialogUpdate = false;
