    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in SMART_KITCHEN_COUNTERS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.smart_kitchen_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.smart_kitchen_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Smart Kitchen Counter</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smart Kitchen Counter Name"
                                        name="smart_kitchen_name" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smart Kitchen Counter Value"
                                        name="smart_kitchen_value" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Smart Kitchen Counter</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smart_kitchen_name"
                                        outlined 
                                        dense 
                                        label="Smart Kitchen Counter Name"
                                        name="smart_kitchen_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smart_kitchen_value"
                                        outlined 
                                        dense 
                                        label="Smart Kitchen Counter Value"
                                        name="smart_kitchen_value" 
        ...
        <app-footer :items="SMART_KITCHEN_COUNTERS.length"></app-footer>
    </div>
</template>
<script>
...
import FloatingButtons from '../../modules/FloatingButtons.vue';
    export default {
        ...
        data: () => ({
            toolbar: {
                title: 'HP SMART KITCHEN COUNTERS',
                view: true,
                back: true,
                filter: false,
                master: false,
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SMART_KITCHEN_COUNTERS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/smart_kitchen_counters/store',
                        data: formdata
                    }).then(() => {
                        this._getSmartKitchenCounters();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/smart_kitchen_counters/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSmartKitchenCounters();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/smart_kitchen_counters/delete',
