    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in SMILE_CUBE_STAIRS" :key="i">
                        <td v-show="editMode">
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title><span class="overline">Create Smile Cube Stair</span></v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field
                                        outlined
                                        dense
                                        label="Smile Cube Stair Name"
                                        name="smile_cube_name"
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field
                                        outlined
                                        dense
                                        label="Smile Cube Stair Value"
                                        name="smile_cube_value"
                                        class="required"
                                        :rules="rules.required">
                                    </v-text-field>
                                </v-col>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Smile Cube Stair</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smile_cube_name"
                                        outlined 
                                        dense 
                                        label="Smile Cube Stair Name"
                                        name="smile_cube_name" 
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smile_cube_value"
                                        outlined 
                                        dense 
                                        label="Smile Cube Stair Value"
                                        name="smile_cube_value" 
        ...
        <app-footer :items="SMILE_CUBE_STAIRS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SMILE_CUBE_STAIRS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/smile_cube_stairs/store',
                        data: formdata
                    }).then(() => {
                        this._getSmileCubeStairs();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/smile_cube_stairs/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSmileCubeStairs();
                        this.dialogUpdate = false;
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/smile_cube_stairs/delete',
