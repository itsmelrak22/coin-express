    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(smartGrandStair, i) in SMART_GRAND_STAIR" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="smartGrandStair.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(smartGrandStair)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ smartGrandStair.smart_grand_value }} </td>
                        <td> {{ smartGrandStair.smart_grand_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(smartGrandStair.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(smartGrandStair.updated_at) }} </td>
                        <td v-show="viewMode"> {{ smartGrandStair.created_by }} </td>
                        <td v-show="viewMode"> {{ smartGrandStair.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smart Grand Stair Code"
                                        name="smart_grand_value" 
                                        class="required"
                                ...
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Smart Grand Stair Name"
                                        name="smart_grand_name" 
                                        class="required"
                                        :rules="rules.required"
                                    > </v-text-field>
                                </v-col>
                    ...
                    <v-card-title> <span class="overline">Edit Sash Type</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smart_grand_value"
                                        outlined 
                                        dense 
                                        label="Smart Grand Stair Code"
                                        name="smart_grand_value" 
                               ...
                               <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.smart_grand_name"
                                        outlined 
                                        dense 
                                        label="Smart Grand Stair Name"
                                        name="smart_grand_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SMART_GRAND_STAIR.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Delete(){
                axios({
                    method: 'POST',
                    url: '/api/smartGrandStair/delete',
                    data: {ids : this.selectedData}
                }).then(() => {
                    this._getSmartGrandStair();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/smartGrandStair/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getSmartGrandStair();
                        this.dialogUpdate = false;
