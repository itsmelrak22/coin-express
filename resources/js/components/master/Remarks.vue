    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(remark, i) in REMARKS" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="remark.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(remark)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ remark.name }} </td>
                        <td v-show="viewMode"> {{ formatDate(remark.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(remark.updated_at) }} </td>
                        <td v-show="viewMode"> {{ remark.created_by }} </td>
                        <td v-show="viewMode"> {{ remark.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        ...
                                        name="name" 
                                        class="required"
                                        :rules="rules.uniqueRemarks(REMARKS)"
                                    > </v-text-field>
                                </v-col>
                            </v-row>
                        </v-card-text>
        ...
        <app-footer :items="REMARKS.length"></app-footer>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.REMARKS.filter(res => res.selected).map(res => res.id); 
            }
        },
