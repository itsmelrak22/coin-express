    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(sprayShipo, i) in SPRAY_SHIPO" :key="i">
                         <td v-show="editMode">
                            <v-checkbox v-model="sprayShipo.selected" dense></v-checkbox>
                        </td>
                        <td v-show="editMode"> 
                            <v-btn
                                fab
                                x-small
                                @click="toggleUpdateDialog(sprayShipo)"
                            >
                                <v-icon >mdi-pencil</v-icon> 
                            </v-btn>
                        </td>
                        <td> {{ sprayShipo.shipo_name }} </td>
                        <td v-show="viewMode"> {{ formatDate(sprayShipo.created_at) }} </td>
                        <td v-show="viewMode"> {{ formatDate(sprayShipo.updated_at) }} </td>
                        <td v-show="viewMode"> {{ sprayShipo.created_by }} </td>
                        <td v-show="viewMode"> {{ sprayShipo.updated_by }} </td>
                    </tr>
                </tbody>
            </v-simple-table>
        <!-- mainTable End -->
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        outlined 
                                        dense 
                                        label="Spray Shipo Stone Atsushi Name"
                                        name="shipo_name" 
                                        class="required"
...
import FloatingButtons from '../../modules/FloatingButtons.vue';
    export default {
        ...
        data: () => ({
            toolbar: {
                title: 'SPRAY SHIPO STONE ATSUSHI',
                view: true,
                back: true,
                filter: false,
                master: false,
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.SPRAY_SHIPO.filter(res => res.selected).map(res => res.id); 
            }
        },
