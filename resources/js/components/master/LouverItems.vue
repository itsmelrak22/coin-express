    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in LOUVER_ITEMS" :key="i">
                        <td v-show="editMode">
                        ...
                        <td> {{ ITEM.louver_name }} </td>
                        <td v-show="viewMode"> {{ ITEM.louver_value }} </td>
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Louver Item</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Louver Item</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.louver_name"
                                        outlined 
                                        dense 
                                        label="Louver Name"
                                        name="louver_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.LOUVER_ITEMS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/louver_items/store',
                        data: formdata
                    }).then(() => {
                        this._getLouverItems();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/louver_items/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getLouverItems();
                        this.dialogUpdate = false;
