    ...
    <div>
        ...
        <!-- mainTable Start -->
            <v-simple-table class="mainTable" fixed-header height="75vh">
                ...
                <tbody>
                    <tr v-for="(ITEM, i) in LOUVER_WOODS" :key="i">
                        <td v-show="editMode">
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create Louver Wood</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Edit Louver Wood</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        v-model="tempData.wood_name"
                                        outlined 
                                        dense 
                                        label="Wood Name"
                                        name="wood_name" 
    ...
    export default {
        ...
        computed : {
            ...
            selectedData(){
                ...
                return this.LOUVER_WOODS.filter(res => res.selected).map(res => res.id); 
            }
        },
        ...
        methods: {
            ...
            Store(){
                if(this.$refs.Store.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: '/api/louver_woods/store',
                        data: formdata
                    }).then(() => {
                        this._getLouverWoods();
            ...
            Update(){
                if(this.$refs.Update.validate()){
                    ...
                    axios({
                        method: 'POST',
                        url: `/api/louver_woods/update/${id}`,
                        data: formdata
                    }).then(() => {
                        this._getLouverWoods();
                        this.dialogUpdate = false;
