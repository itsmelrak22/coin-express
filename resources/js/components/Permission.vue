...
<!-- ################################# DIALOGS #################################-->
        ...
        <v-dialog v-model="dialogStore" max-width="300" persistent>
            <v-form id="Store" ref="Store" @submit.prevent="Store">
                <v-card>
                    <v-card-title> <span class="overline">Create New Permission</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field 
                                        ...
                                        name="name" 
                                        class="required"
                                        :rules="rules.uniqueData(PERMISSIONS)"
                                    > </v-text-field>
                                </v-col>
                            </v-row>
                        </v-card-text>
        ...
        <v-dialog v-model="dialogUpdate" max-width="300" persistent>
            <v-form id="Update" ref="Update" @submit.prevent="Update">
                <v-card>
                    <v-card-title> <span class="overline">Update Permission</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field
                                        ...
                                        name="name" 
                                        class="required"
                                        :rules="rules.uniqueDataEdit(PERMISSIONS, tempData.name)"
                                    > </v-text-field>
                                </v-col>
                            </v-row>
                        </v-card-text>
                ...
                <v-card>
                    <v-card-title> <span class="overline">Delete Permission</span> </v-card-title>
                        <v-card-text>
                            <v-row>
                                <v-col cols="12">
                                    <p class="overline">CONFIRM DELETE PERMISSION?</p>
                                </v-col>
                            </v-row>
                        </v-card-text>
                    <v-card-actions>
...
export default {
    ...
    methods: {
        ...
        Store(){
            if(this.$refs.Store.validate()){
                ...
                axios({
                    method: 'POST',
                    url: '/api/master/permission/store',
                    data: formdata
                }).then(() => {
        ...
        Update(){
            if(this.$refs.Update.validate()){
                ...
                axios({
                    method: 'POST',
                    url: `/api/master/permission/update/${this.tempData.id}`,
                    data: formdata
                }).then(() => {
                    this._getPermissions();
        ...
        Delete(){
            if(this.$refs.Delete.validate()){
                ...
                axios({
                    method: 'POST',
                    url: `/api/master/permission/delete/${this.tempData.id}`,
                    data: formdata
                }).then(() => {
                    this._getPermissions();
    ...
    async mounted(){
        this.overlay = true;
        await this._getPermissions().then(() => {
            this.overlay = false
        })
    }
